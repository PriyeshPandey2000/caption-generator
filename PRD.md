...
- **Timeline range-select — maybe, not committed.** Drag-select a range of caption groups directly on the `Timeline` (not just words within one visible group on canvas), feeding the same bulk-edit Inspector panel the canvas marquee already uses. Sourced from real user complaints (r/editors thread, 2026-09-07): VEED's #1 unaddressed request is exactly this ("a chunk of 20 subs I want to look different — no way to edit just those 20"), acknowledged but deprioritized by VEED's own CEO. Design resolved but unbuilt: distinguish by drag **start location**, not a mode toggle — dragging the playhead handle scrubs (a capability that doesn't exist today either), dragging empty track background range-selects, word-block click/edge-drag stay as they are now. Also would need a bulk-**reposition** story (position is currently per-caption-group, not per-word) — deliberately scoped out of a first pass to avoid shipping two new interaction concepts at once.
- **Savable custom presets — maybe, not committed.** VEED's most-praised capability (per the same thread) and currently entirely absent from CaptionLab. Proposed as a low-visibility "Save current style as preset" text link in the existing Presets panel, rendered as the same card component under a "Your Presets" divider, stored in `localStorage` **globally** (not per-project) so a saved preset carries across videos.
- **Player transport controls — not committed.** Replace the plain-text "Play" button (bottom-left of preview) with a real transport control: an icon-only `▶` toggle (⇄ `⏸` when playing), or with a time readout `▶ 00:12 / 02:01`. Compact player toolbar proposed as: `↶  ▶  ↷   00:12 / 02:01   🔊,` read as **skip-back / play–pause / skip-forward + elapsed/total + speaker/mute volume**. Add a small playback-speed menu: `0.5×  1×  1.5×  2×`. Rule: **never render plain-text "Play"** — it reads as unfinished placeholder next to the rest of the editor's polished controls.
- **Export dropdown — not committed.** Consolidate the three separate `SRT / VTT / Export MP4` buttons in the header into a single `Export ▾` dropdown — one button, three items inside, so the header doesn't advertise three export "buttons" at once and the primary action (Export MP4) stays the obvious default:
  ```
  Export ▾
  ──────────────
  Export MP4
  ...
  Export VTT
  ```
  Clicking the main button exports MP4 directly; the ▾ menu is for SRT/VTT (and, later, any additional export formats).
